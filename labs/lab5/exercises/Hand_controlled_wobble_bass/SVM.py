    ...
    for img_name in os.listdir(class_a_path):
        if not img_name.startswith('.'):
            ...
            if generate_arrays:
                ...
                images_vector = np.concatenate(
                    (images_vector, img_vector), axis=0)
...
def train_svm():
    ...
    images, images_vector, labels = load_data(
        class_a_path='images/class_a/', class_b_path='images/class_b/')
    ...
    with warnings.catch_warnings():
        ...
        xtrain, xtest, ytrain, ytest = train_test_split(
            images_vector, labels, random_state=42)
